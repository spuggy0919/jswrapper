...
typedef void (*TWOWIRE_CALLBACK0)(); // define call back type 
static js_value_t jscallback0 = null;
typedef void (*TWOWIRE_CALLBACK1)(int);
static js_value_t jscallback1 = null;
...
JS_METHOD(twowire_slavebegin)
{
    ...
    if(native_pointer) {
        TwoWire* twowire = (TwoWire*)native_pointer;
        return jerry_boolean(twowire->slavebegin((uint8_t) slaveAddr,(int) sda,(int) scl,(uint32_t) frequency));
    }
    return jerry_undefined();
